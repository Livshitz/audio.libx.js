...
export class AudioCache {
	 ...
	 */
	public async set(id: string, chunks: Uint8Array[], mimeType: string): Promise<void> {
		if (!this._db) {
	...
	private _updateStats(): void {
		...
		this._stats.totalSize = Array.from(this._memoryCache.values())
	...
	/**
	 * Cleanup old entries based on age or access frequency
	 */
	public async cleanup(options: {
		...
		// Sort by access count (descending) and age (ascending)
		const sortedEntries = entries.sort((a, b) => {
			const aAccess = this._accessCounts.get(a.id) || 0;
			const bAccess = this._accessCounts.get(b.id) || 0;

			if (aAccess !== bAccess) {
				return bAccess - aAccess; // Higher access count first
			}

			return a.cachedAt - b.cachedAt; // Older entries first
		});
		...
		for (const entry of sortedEntries) {
			...
			if (options.minAccessCount) {
				const accessCount = this._accessCounts.get(entry.id) || 0;
				if (accessCount < options.minAccessCount) {
					shouldDelete = true;
				}
			}

			// Check max entries (delete least accessed/oldest)
			if (options.maxEntries && (entries.length - deletedCount) > options.maxEntries) {
				shouldDelete = true;
			}
